module.exports.emmaDiceDato = async function (lastChannel, message) {
  ...
  function takeRandomGuy() {
    ...
    do {
      randomGuy = lastChannel.members.random();
    } while (randomGuy.user.bot);
  ...
  function enviarDato() {
    ...
    switch (filter) {
      case 0: {
        message.reply(datoEnviar.message);
        break;
      }
      case 1: {
        message.reply(datoEnviar.message + guyOne + datoEnviar.messageTwo);
        break;
      }
      case 2: {
        message.reply(
          datoEnviar.message +
            guyOne +
            datoEnviar.messageTwo +
            guyTwo +
            datoEnviar.messageThree
        );
        break;
      }
    }
